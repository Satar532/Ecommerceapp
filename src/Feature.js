...
reducers:{
 ...
 Addtocart:(stata,acton)=>{
  ...
  removeLoginOut:(state)=>{
   state.Alreadyacc=false;
  } ,
 updateqty:(state,acation)=>{
     const {id,qty}=acation.payload
       if(qty<1) return ;
    const currentusers=state.users.find(curr=>curr.name===state.currentuser)
      const item=currentusers.Carts.find(item=>item.id==id)
 ...
 Remove:(state,acton)=>{
      const confirm=window.confirm("do u really wanto to remove")
       if(!confirm) return;
    const user=state.users.find(curr=>curr.name===state.currentuser)
           user.Carts= user.Carts.filter(item=>item.id!==acton.payload)
 ...
 Rangeapply:(state,action)=>{
     
     state.rangeData= state.PrductList.filter(item=>item.price<=action.payload||item.category===action.payload)

 },
 SortlowtoHigh:(state,acton)=>{
  console.log(acton.payload)
  if(acton.payload==='low to high'){
    state.rangeData = state.PrductList.sort((a,b)=>a.price-b.price)
    
  }
  if(acton.payload==='high to low'){
    state.rangeData= state.PrductList.sort((a,b)=>b.price-a.price)
...
setprofileimage:(state,action)=>{
  if(!state.isLogined) return alert("please first login")
 const img=state.users.find(user=>user.name===state.currentuser);
 if(img)img.primage=action.payload 
  alert("you have changed profile image")  
}
